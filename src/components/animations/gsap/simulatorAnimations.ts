...
export const animateSimulatorSection = (section: HTMLElement) => {
  ...
  return buildSectionTimeline(section, 'top 72%', 'top 48%')
    ...
    .fromTo(circle, { autoAlpha: 0, scale: 0.92, transformOrigin: 'center center' }, { autoAlpha: 1, scale: 1, duration: typeof window !== 'undefined' && window.innerWidth <= 768 ? 0.72 : 1, ease: 'power3.out' }, '-=0.18')
    .fromTo(panel, { autoAlpha: 0, x: 34 }, { autoAlpha: 1, x: 0, duration: typeof window !== 'undefined' && window.innerWidth <= 768 ? 0.62 : 0.84, ease: 'power3.out' }, '-=0.66')
